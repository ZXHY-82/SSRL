...
parser.add_argument('--label_fixed_epochs', default=0, type=int)
parser.add_argument('--laebl_update_epochs', default=5, type=int)
...
parser.add_argument('--freeze_ptm', default=False, type=str2bool)
...
def main_worker(gpu, args):
    ...
    noise_list = {'noise': [i.strip('\n') for i in open('data/' + envir_dir +'/noise_wav_list')],
                  'music': [i.strip('\n') for i in open('data/' + envir_dir +'/music_wav_list')],
                  'babb': [i.strip('\n') for i in open('data/' + envir_dir +'/speech_wav_list')],
                  'reverb': [i.strip('\n') for i in open('data/' + envir_dir +'/simu_rir_list')]}
    ...
    ema_teacher = SpeakerModel(feat_dim=args.feat_dim, embd_dim=args.embd_dim, dropout=0.5, num_spks=num_spks, arcface=args.arcface, layer_nums=args.layer_nums)
    ...
    if args.distributed:
        ...
        model_ptm = torch.nn.parallel.DistributedDataParallel(model_ptm,find_unused_parameters=True, device_ids=[args.gpu])
    else:
        model = model.cuda(args.gpu)
        ema_teacher = ema_teacher.cuda(args.gpu)
        model_ptm = model_ptm.cuda(args.gpu)
        
    checkpoint = torch.load('/Netdata/2023/lize/ASV/SSRL/exp/dino_ali_lnc_nlm_8k_wavlm_v2_layer24_stage2_70/model_72.pkl', map_location='cpu')
    ...
    if args.freeze_ptm:
        ...
        optimizer = torch.optim.Adam(list(model_ptm.parameters()) + list(model.parameters()), lr=args.lr, weight_decay=args.wd)

    scheduler = LR_Scheduler(optimizer, args.warm_up_epoch,
                             args.lr/10, args.lr/100, args.label_fixed_epochs,
                             args.lr/20, args.laebl_update_epochs, len(trn_loader))


    trn_loader.dataset.aug_rate = 1.0
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss, prec, new_labels, teacher_loss = train(trn_loader, model_ptm, model, ema_teacher, criterion, optimizer, scheduler, epoch, is_update, args,tst_loader,val_utt,eer_cal)
        results = 'Epoch %3d  LR %.5f  ' % (epoch, scheduler.get_lr()) + \
                  'Loss %s  ' % (loss) + \
                  'Accuracy %s  ' % (prec)

        embd = test(tst_loader, model_ptm,ema_teacher.ema_model, args)[:len(val_utt)]
...
def train(trn_loader, model_ptm, model, ema_teacher, criterion, optimizer, scheduler, epoch, is_update, args,tst_loader,val_utt,eer_cal):
    ...
    for i, (feats, feats_clean, label, p_clean, idx) in enumerate(trn_loader):
        ...
        if epoch >= args.warm_up_epoch + args.label_fixed_epochs - (args.laebl_update_queue_len//2+1):
            model_ptm.eval()
            with torch.no_grad():
                feats_clean = model_ptm(feats_clean.cuda(args.gpu))
        ...
        if (i+1) % 500 == 0:
            ...
            if args.rank == 0:
                eer_cal.update_embd(embd)
                eer, cost = eer_cal.eer_cost()
                print('EER %2.4f DCF %.4f' % (eer, cost))
                torch.save({'model': model.state_dict(),
                        'ema_teacher': ema_teacher.state_dict(),
                        'model_ptm': model_ptm.state_dict(),
                        'optimizer': optimizer.state_dict(),
                        ...
                        'utt2label': trn_loader.dataset.utt2label,
                        # 'teacher_loss': teacher_loss,
                       }, 'exp/%s/model_%d_%ditems.pkl' % (args.save_dir, epoch,i))
                torch.save({'random': random.getstate(),
                            'np': np.random.get_state(),
                            'torch': torch.get_rng_state()
                        }, 'exp/%s/random_state.pkl' % args.save_dir)
