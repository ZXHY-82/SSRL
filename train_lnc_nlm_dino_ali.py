...
# nohup python train_lnc_nlm_dino_ali.py --save_dir dino_ali_lnc_nlm_8k --gpu 3,4 --port 8441 > dino_ali_lnc_nlm_8k.log & 
# nohup python train_lnc_nlm_dino_ali.py --save_dir dino_ali_lnc_nlm_8k_aam --gpu 3,4 --port 8441 --arcface True >> dino_ali_lnc_nlm_8k_aam.log & 
...
parser.add_argument('-b', '--batch_size', default=480, type=int)
# test dataset
...
parser.add_argument('--label_fixed_epochs', default=5, type=int)
parser.add_argument('--laebl_update_epochs', default=100, type=int)
...
def main():
    ...
    args.epochs = args.warm_up_epoch + args.label_fixed_epochs + args.laebl_update_epochs
...
def main_worker(gpu, args):
    ...
    model = SpeakerModel(feat_dim=args.feat_dim, embd_dim=args.embd_dim, dropout=0.5, num_spks=num_spks, arcface=args.arcface)
    ckp = torch.load('exp/dino/model.pkl', map_location='cpu')['criterion']
    for i, j in ckp.items():
        if 'module.' in i:
            ckp = {i[7:]:j for i, j in ckp.items()}
            break
    ckp = {i.replace('ema_model.backbone.', ''):j for i, j in ckp.items() if i.startswith('ema_model.backbone.')}
    model.encoder.load_state_dict(ckp)

    centers = np.load(f'exp/dino/vox2dev_embds_kmeans_{num_spks}_centers.npy')
    model.classifier.weight.data = torch.tensor(centers, dtype=torch.float32)
    if not args.arcface:
        model.classifier.bias.data.zero_()

    ema_teacher = SpeakerModel(feat_dim=args.feat_dim, embd_dim=args.embd_dim, dropout=0.5, num_spks=num_spks, arcface=args.arcface)
    ema_teacher.load_state_dict(model.state_dict())
    ema_teacher = EMAteacher(ema_teacher, ema_decay=args.ema_decay, ema_anneal_end_step=len(trn_loader)*args.epochs)

    if args.distributed:
        model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)
        model.cuda(args.gpu)
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu])
        ema_teacher = torch.nn.SyncBatchNorm.convert_sync_batchnorm(ema_teacher)
        ema_teacher.cuda(args.gpu)
    else:
        model = model.cuda(args.gpu)
        ema_teacher = ema_teacher.cuda(args.gpu)
    
    criterion = nn.CrossEntropyLoss(reduction='none').cuda()
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.wd)

    scheduler = LR_Scheduler(optimizer, args.warm_up_epoch,
                             args.lr/10, args.lr/100, args.label_fixed_epochs,
                             args.lr/2, args.laebl_update_epochs, len(trn_loader))
    
    # optionally resume from a checkpoint
    if args.start_epoch != 0:
        checkpoint = torch.load('exp/%s/model_%d.pkl' % (args.save_dir, args.start_epoch-1), map_location='cpu')
        model.load_state_dict(checkpoint['model'])
        ema_teacher.load_state_dict(checkpoint['ema_teacher'])
        optimizer.load_state_dict(checkpoint['optimizer'])
        trn_loader.dataset.utt2spk_evolve = checkpoint['utt2spk_evolve']
        trn_loader.dataset.utt2label = checkpoint['utt2label']
        all_label = trn_loader.dataset.utt2label.values()
        trn_loader.dataset.label_to_remove = list(trn_loader.dataset.original_labels - set(all_label))
        trn_loader.dataset.clean_label_prob = checkpoint['clean_label_prob']
        scheduler.iter = args.start_epoch * len(trn_loader)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        is_update = epoch+1 >= args.warm_up_epoch + args.label_fixed_epochs and \
                    (epoch+1 - args.warm_up_epoch - args.label_fixed_epochs) % args.laebl_update_freq == 0

        loss, prec, new_labels, teacher_loss = train(trn_loader, featCal, model, ema_teacher, criterion, optimizer, scheduler, epoch, is_update, args)
        results = 'Epoch %3d  LR %.5f  ' % (epoch, scheduler.get_lr()) + \
                  'Loss %s  ' % (loss) + \
                  'Accuracy %s  ' % (prec)

        embd = test(tst_loader, ema_teacher.ema_model, featCal, args)[:len(val_utt)]
        ...
        results += trn_loader.dataset.update_labels(new_labels, update=is_update)
        results += f'  {is_update}  '

        if epoch >= args.warm_up_epoch + args.label_fixed_epochs - (args.laebl_update_queue_len//2+1):
            results += trn_loader.dataset.update_clean_label_probability(teacher_loss)
        ...
        if args.rank == 0:
            torch.save({'model': model.state_dict(),
                        ...
                        'utt2spk_evolve': trn_loader.dataset.utt2spk_evolve,
                        'clean_label_prob': trn_loader.dataset.clean_label_prob,
                        'utt2label': trn_loader.dataset.utt2label,
                        'teacher_loss': teacher_loss,
                       }, 'exp/%s/model_%d.pkl' % (args.save_dir, epoch))
...
def train(trn_loader, featCal, model, ema_teacher, criterion, optimizer, scheduler, epoch, is_update, args):
    ...
    for i, (feats, feats_clean, label, p_clean, idx) in enumerate(trn_loader):

        data_time.update(time.time() - end)

        label = label.cuda(args.gpu)
        idx = idx.cuda(args.gpu)
        p_clean = p_clean.cuda(args.gpu)

        if epoch >= args.warm_up_epoch + args.label_fixed_epochs - (args.laebl_update_queue_len//2+1):
            with torch.no_grad():
                feats_clean = featCal(feats_clean.cuda(args.gpu))
                outputs = ema_teacher.ema_model(feats_clean, label)

                if args.ignore_removed_label_in_teacher:
                    outputs[:, indices_to_ignore] = -float('inf')

                if args.use_sinkhorn:
                    self_label = Sinkhorn(F.softmax(outputs, dim=1)).argmax(dim=1)
                else:
                    self_label = outputs.argmax(dim=1)
                new_label = trn_loader.dataset.infer_new_labels(zip(idx.tolist(), self_label.tolist()), is_update)
                loss = criterion(outputs, torch.LongTensor(new_label).cuda(args.gpu))
                
                idx = concat_all_gather(idx).reshape(-1).tolist()
                self_label = concat_all_gather(self_label).reshape(-1).tolist()
                loss = concat_all_gather(loss).reshape(-1).tolist()
                teacher_loss += [(int(i), float(l)) for i, l in zip(idx, loss)]
                self_labels += [(int(i), int(l)) for i, l in zip(idx, self_label)]
    
        feats = featCal(feats.cuda(args.gpu))
        outputs = model(feats, label=label)
        outputs[:, indices_to_ignore] = -float('inf')

        if args.noisy_label_mnodeling:
            loss = (criterion(outputs, label) * p_clean).sum() / p_clean.sum()
        else:
            loss = criterion(outputs, label).mean()

        losses.update(loss.item(), feats.size(0))
        top1.update(accuracy(outputs, label)[0].item(), feats.size(0))

        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        scheduler.step()
        ema_teacher.ema_step(model)
    ...
    if args.distributed:
        ...
        return '%.4f' % (loss/args.ngpus), '%3.2f' % (prec1/args.ngpus), self_labels, teacher_loss
    else:
        return '%.4f' % (losses.avg), '%3.2f' % (top1.avg), self_labels, teacher_loss
...
class SpeakerModel(nn.Module):
    def __init__(self, feat_dim=80, embd_dim=128, dropout=0, num_spks=6000, arcface=False):
