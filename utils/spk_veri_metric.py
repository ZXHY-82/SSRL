...
class SVevaluation(object):
    ...
    def eer_cost(self, score_norm=None, num_cohort=None):
        if score_norm == 'asnorm1':
            scores = [self.score_norm((self.embd[i] * self.embd[j]).sum(),
                                      self.score_cohort[i, self.idx_cohort[i, :num_cohort]],
                                      self.score_cohort[j, self.idx_cohort[j, :num_cohort]])
                      for i, j in self.trial_idx]
        elif score_norm == 'asnorm2':
            scores = [self.score_norm((self.embd[i] * self.embd[j]).sum(),
                                      self.score_cohort[i, self.idx_cohort[j, :num_cohort]],
                                      self.score_cohort[j, self.idx_cohort[i, :num_cohort]])
                      for i, j in self.trial_idx]
        else:
...
class SVevaluationC(object):
    def __init__(self, trial_file, utt, embd):
        self.embd = length_norm(embd)
        self.scores = None

        utt_idx = {u:i for i, u in enumerate(utt)}
        self.trial_idx = [[utt_idx.get(line.split()[0]), utt_idx.get(line.split()[1])] for line in open(trial_file)]
        bad_idx = [i for i, ti in enumerate(self.trial_idx) if None in ti]
    ...
    def get_score(self, score_norm=None, num_cohort=None):
        if score_norm == 'asnorm1':
            scores = [self.score_norm((self.embd[i] * self.embd[j]).sum(),
                                      self.score_cohort[i, self.idx_cohort[i, :num_cohort]],
                                      self.score_cohort[j, self.idx_cohort[j, :num_cohort]])
